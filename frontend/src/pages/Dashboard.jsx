...
export default function Dashboard() {
 ...
 let getUserData = async()=>{
    try {
      ...
      let result =await axios.get(`/users/getUserById/${id}`,{withCredentials:true,headers:{
        authorization:`Bearer ${localStorage.getItem('jwt')}`
      }});
      console.log(result.data);
      setUser(result.data);
      setImageUrl(`/users/getimage/${result.data?.user_profile}`)
  ...
  let uploadProfile = async()=>{
      ...
      try {
        let result = await axios.post(`/users/addProfile`,formData,{headers:{
